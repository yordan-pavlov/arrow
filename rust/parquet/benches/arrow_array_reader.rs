...
use parquet::{arrow::array_reader::ArrayReader, basic::Encoding, column::page::PageIterator, data_type::ByteArrayType, schema::types::{ColumnDescPtr, SchemaDescPtr}};

fn build_test_schema() -> SchemaDescPtr {
    use parquet::schema::{types::SchemaDescriptor, parser::parse_message_type};
    let message_type = "
        message test_schema {
            REQUIRED BYTE_ARRAY mandatory_leaf (UTF8);
            REPEATED Group test_mid {
                OPTIONAL BYTE_ARRAY optional_leaf (UTF8);
            }
        }
        ";
...
fn create_arrow_array_reader(page_iterator: impl PageIterator + 'static, column_desc: ColumnDescPtr) -> impl ArrayReader {
...
fn create_complex_array_reader(page_iterator: impl PageIterator + 'static, column_desc: ColumnDescPtr) -> impl ArrayReader {
...
fn add_benches(c: &mut Criterion) {
    ...
    let mandatory_string_column_desc = schema.column(0);
    ...
    group.bench_function("read StringArray, plain encoded, mandatory, no NULLs - new", |b| b.iter(|| {
        let array_reader = create_arrow_array_reader(plain_string_no_null_data.clone(), mandatory_string_column_desc.clone());
        count = bench_array_reader(array_reader);
    }));
    println!("read {} values", count);
    ...
    group.bench_function("read StringArray, plain encoded, optional, no NULLs - old", |b| b.iter(|| {
        let array_reader = create_complex_array_reader(plain_string_no_null_data.clone(), optional_string_column_desc.clone());
        count = bench_array_reader(array_reader);
    }));
    println!("read {} values", count);

    group.bench_function("read StringArray, plain encoded, optional, no NULLs - new", |b| b.iter(|| {
        let array_reader = create_arrow_array_reader(plain_string_no_null_data.clone(), optional_string_column_desc.clone());
        count = bench_array_reader(array_reader);
    }));
    println!("read {} values", count);
    ...
    group.bench_function("read StringArray, plain encoded, optional, half NULLs - old", |b| b.iter(|| {
        let array_reader = create_complex_array_reader(plain_string_half_null_data.clone(), optional_string_column_desc.clone());
        count = bench_array_reader(array_reader);
    }));
    println!("read {} values", count);

    group.bench_function("read StringArray, plain encoded, optional, half NULLs - new", |b| b.iter(|| {
        let array_reader = create_arrow_array_reader(plain_string_half_null_data.clone(), optional_string_column_desc.clone());
        count = bench_array_reader(array_reader);
    }));
    println!("read {} values", count);
    ...
    group.bench_function("read StringArray, dictionary encoded, mandatory, no NULLs - old", |b| b.iter(|| {
        let array_reader = create_complex_array_reader(dictionary_string_no_null_data.clone(), mandatory_string_column_desc.clone());
        count = bench_array_reader(array_reader);
    }));
    println!("read {} values", count);

    group.bench_function("read StringArray, dictionary encoded, mandatory, no NULLs - new", |b| b.iter(|| {
        let array_reader = create_arrow_array_reader(dictionary_string_no_null_data.clone(), mandatory_string_column_desc.clone());
        count = bench_array_reader(array_reader);
    }));
    println!("read {} values", count);
    ...
    group.bench_function("read StringArray, dictionary encoded, optional, no NULLs - old", |b| b.iter(|| {
        let array_reader = create_complex_array_reader(dictionary_string_no_null_data.clone(), optional_string_column_desc.clone());
        count = bench_array_reader(array_reader);
    }));
    println!("read {} values", count);

    group.bench_function("read StringArray, dictionary encoded, optional, no NULLs - new", |b| b.iter(|| {
        let array_reader = create_arrow_array_reader(dictionary_string_no_null_data.clone(), optional_string_column_desc.clone());
        count = bench_array_reader(array_reader);
    }));
    println!("read {} values", count);
    ...
    group.bench_function("read StringArray, dictionary encoded, optional, half NULLs - old", |b| b.iter(|| {
        let array_reader = create_complex_array_reader(dictionary_string_half_null_data.clone(), optional_string_column_desc.clone());
        count = bench_array_reader(array_reader);
    }));
    println!("read {} values", count);

    group.bench_function("read StringArray, dictionary encoded, optional, half NULLs - new", |b| b.iter(|| {
        let array_reader = create_arrow_array_reader(dictionary_string_half_null_data.clone(), optional_string_column_desc.clone());
        count = bench_array_reader(array_reader);
    }));
    println!("read {} values", count);
